...
bool
CFI::putFDEorCIE( DWARFReader &reader, FDE **fdep ) {
   ...
   *fdep = nullptr;
   ...
   if (associatedCIE == Elf::Off(-1)) {
      ...
      if (cies.find(associatedCIE) == cies.end()) {
         ...
         FDE *notAnFde;
         putFDEorCIE(r2, &notAnFde);
         assert(notAnFde == nullptr);
      }
    ...
    , type(type_)
{
    ...
    do {
       ...
       auto ptr_enc = hdr.getu8();
       auto fde_count_enc = hdr.getu8();
       ...
       if ( enc == DW_EH_PE_omit || (0xf & fde_count_enc ) == DW_EH_PE_omit )
          break;
       ...
       decodeAddress( hdr, ptr_enc, ehFrameHdrSec.shdr.sh_addr );
       bool indirectTable;
       std::tie( fdeTableSize, indirectTable )= decodeAddress( hdr, fde_count_enc, 0);
    ...
    while (!reader.empty() && putFDEorCIE(reader, nullptr))
       ;
}
...
CFI::findFDE(Elf::Addr addr)
{
   ...
   uintptr_t start = 0;
   uintptr_t end = fdeTableSize;
   uintptr_t closestAddr = 0, closestFde = 0;

   size_t encodingSize = sizeForEncoding( ExceptionHandlingEncoding(fdeTableEnc) );

   size_t entrySize = encodingSize * 2;

   DWARFReader r( fdeTable );
   while (start < end) {
      auto mid = start + (end - start) / 2;
      r.setOffset( entrySize * mid );
      auto [fdeAddr,indirecAddr] = decodeAddress(r, fdeTableEnc, ehFrameHdrAddr);
      auto [fdeOff,indirectOff] = decodeAddress(r, fdeTableEnc, ehFrameHdrAddr);
      if (fdeAddr <= addr) {
         start = mid + 1;
         if ( fdeAddr > closestAddr ) {
            closestAddr = fdeAddr;
            closestFde = fdeOff;
         }
         if (addr == fdeAddr)
   ...
   // The closestFde is a virtual address. Make it relative to the eh_frame section.
   // the eh frame section. 
   closestFde -= sectionAddr;
   fdeReader.setOffset( closestFde );
   FDE *fde;
   if ( putFDEorCIE( fdeReader, &fde )) {
      return fde;
   } else {
      return nullptr;
   }
